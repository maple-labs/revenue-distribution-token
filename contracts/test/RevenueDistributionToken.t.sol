...
contract TestFail is TestUtils {
    ...
    function test_fail() external {
        ...
        emit log_named_uint("rdToken.previewDeposit(depositAmount)", rdToken.previewDeposit(1));
        ...
        emit log_named_uint("rdToken.totalAssets()                 ", rdToken.totalAssets());
        emit log_named_uint("rdToken.totalSupply()                 ", rdToken.totalSupply());
        emit log_named_uint("rdToken.convertToAssets(shares)       ", rdToken.convertToAssets(shares));
        emit log_named_uint("rdToken.convertToShares(depositAmount)", rdToken.convertToShares(depositAmount));
        emit log_named_uint("rdToken.previewRedeem(depositAmount)  ", rdToken.previewRedeem(depositAmount));
        emit log_named_uint("shares       ", shares);
        emit log_named_uint("depositAmount", depositAmount);

        emit log_named_uint("asset.balanceOf(address(staker)", asset.balanceOf(address(staker)));
