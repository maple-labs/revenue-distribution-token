...
contract RDTTestBase is TestUtils {
    ...
    bytes constant ARITHMETIC_ERROR =
    ...
    function setUp() public virtual {
        ...
        rdToken =
        new RDT("Revenue Distribution Token", "RDT", address(this), address(asset), 1e30);
    ...
    function _transferAndUpdateVesting(
        address asset_,
        address rdToken_,
    ...
    function _getMinDeposit(address rdToken_)
        internal
        view
        returns (uint256 minDeposit_)
    {
        minDeposit_ =
            (RDT(rdToken_).totalAssets() - 1) / RDT(rdToken_).totalSupply() + 1;
    }
...
contract RDTSuccessTestBase is RDTTestBase {
        ...
        internal
    {
        ...
        rdToken_convertToAssets =
            _toInt256(rdToken.convertToAssets(sampleSharesToConvert));
        rdToken_convertToShares =
            _toInt256(rdToken.convertToShares(sampleAssetsToConvert));
        ...
        asset_allowance_staker_rdToken =
            _toInt256(asset.allowance(staker_, address(rdToken)));
    ...
    function _assertMint(address staker_, uint256 mintAmount_, bool fuzzed_)
        internal
    {
    ...
    function _assertDepositWithPermit(
        address staker_,
        uint256 stakerPrivateKey_,
        uint256 depositAmount_,
        bool fuzzed_
    )
        internal
    {
        ...
        _getValidPermitSignature(staker_, address(rdToken), depositAmount_,  block.timestamp, stakerPrivateKey_);
        vm.prank(staker_);
        uint256 shares =
        rdToken.depositWithPermit(depositAmount_, staker_, block.timestamp, v, r, s);
    ...
    function _assertMintWithPermit(
        address staker_,
        uint256 stakerPrivateKey_,
        uint256 mintAmount_,
        bool fuzzed_
        ...
        internal
    {
        ...
        (uint8 v, bytes32 r, bytes32 s) =
        _getValidPermitSignature(staker_, address(rdToken), maxAssets, block.timestamp, stakerPrivateKey_);
        vm.prank(staker_);
        uint256 depositedAmount =
        rdToken.mintWithPermit(mintAmount_, staker_, maxAssets, block.timestamp, v, r, s);
    ...
    function _assertWithdrawCallerNotOwner(
        address caller_,
        address staker_,
        ...
        internal
    {
        rdToken_allowance_staker_caller =
            _toInt256(rdToken.allowance(staker_, caller_));
        rdToken_balanceOf_staker = _toInt256(rdToken.balanceOf(staker_));
        rdToken_totalSupply = _toInt256(rdToken.totalSupply());
        rdToken_freeAssets = _toInt256(rdToken.freeAssets());
        rdToken_totalAssets = _toInt256(rdToken.totalAssets());
        ...
        Staker(caller_).rdToken_withdraw(address(rdToken), withdrawAmount_, caller_, staker_);
    ...
    function _assertRedeemCallerNotOwner(
        address caller_,
        address staker_,
        ...
        internal
    {
        ...
        Staker(caller_).rdToken_redeem(address(rdToken), redeemAmount_, caller_, staker_);
        ...
        internal
    {
        ...
        uint256 sharesBurned =
            Staker(staker_).rdToken_withdraw(address(rdToken), withdrawAmount_);
    ...
    function _assertRedeem(address staker_, uint256 redeemAmount_, bool fuzzed_)
        internal
    {
        rdToken_balanceOf_staker = _toInt256(rdToken.balanceOf(staker_));
        rdToken_totalSupply = _toInt256(rdToken.totalSupply());
        ...
        uint256 fundsWithdrawn =
            Staker(staker_).rdToken_redeem(address(rdToken), redeemAmount_);
    ...
    function _toInt256(uint256 unsigned_)
        internal
        pure
        returns (int256 signed_)
    {
        signed_ = int256(unsigned_);
        require(signed_ >= 0, "TO_INT256_OVERFLOW");
    }

    function _toUint256(int256 signed_)
        internal
        pure
        returns (uint256 unsigned_)
    {
...
contract AuthTests is RDTTestBase {
    ...
    function setUp() public virtual override {
        ...
        asset = new MockERC20("MockToken", "MT", 18);
        rdToken =
        new RDT("Revenue Distribution Token", "RDT", address(owner), address(asset), 1e30);
        vm.warp(START);
    }
...
contract ConstructorTest is TestUtils {
    function test_constructor_ownerZeroAddress() public {
        ...
        RDT rdToken =
        new RDT("Revenue Distribution Token", "RDT", address(0), address(asset), 1e30);
...
contract DepositFailureTests is RDTTestBase {
    ...
    function test_deposit_insufficientBalance(uint256 depositAmount_)
        public
    {
        depositAmount_ = constrictToRange(depositAmount_, 1, 1e29);
    ...
    function test_deposit_zeroShares() public {
        ...
        uint256 minDeposit =
            (rdToken.totalAssets() - 1) / rdToken.totalSupply() + 1;
...
contract DepositTests is RDTSuccessTestBase {
    ...
    function testFuzz_deposit_singleUser_preVesting(uint256 depositAmount_)
        public
    {
        depositAmount_ = constrictToRange(depositAmount_, 1, 1e29);
        ...
        public
    {
        ...
        uint256 expectedShares =
            depositAmount_ * rdToken.totalSupply() / rdToken.totalAssets();
        ...
        public
    {
        ...
        vestingAmount_ = constrictToRange(vestingAmount_, 1,         1e29);
        vestingPeriod_ =
            constrictToRange(vestingPeriod_, 1 seconds, 10_000 days);
        ...
        public
    {
        ...
        for (uint256 i = 1; i < 11; ++i) {
            uint256 depositAmount =
                uint256(keccak256(abi.encodePacked(depositSeed_, i)));
            uint256 warpTime =
                uint256(keccak256(abi.encodePacked(warpSeed_,    i)));

            // Get minimum deposit to avoid ZERO_SHARES.
            uint256 minDeposit = _getMinDeposit(address(rdToken));
            depositAmount =
                constrictToRange(depositAmount, minDeposit, 1e29 + 1); // + 1 since we round up in min deposit.
            warpTime =
                constrictToRange(warpTime,      0,          vestingPeriod_ / 10); // Needs to be smaller than vestingPeriod_ / 10

            vm.warp(block.timestamp + warpTime);

            uint256 expectedShares =
                depositAmount * rdToken.totalSupply() / rdToken.totalAssets();
        ...
        public
    {
        ...
        for (uint256 i = 1; i < 11; ++i) {
            uint256 depositAmount =
                uint256(keccak256(abi.encodePacked(seed_, i)));
...
contract DepositWithPermitFailureTests is RDTTestBase {
    ...
    function test_depositWithPermit_zeroAddress() public {
        ...
        (uint8 v, bytes32 r, bytes32 s) =
        _getValidPermitSignature(staker, address(rdToken), depositAmount, deadline, stakerPrivateKey);
    ...
    function test_depositWithPermit_notStakerSignature() public {
        ...
        (uint8 v, bytes32 r, bytes32 s) =
        _getValidPermitSignature(notStaker, address(rdToken), depositAmount, deadline, notStakerPrivateKey);
...
contract DepositWithPermitTests is RDTSuccessTestBase {
    ...
    function testFuzz_depositWithPermit_singleUser_preVesting(
        uint256 depositAmount_
    )
        public
    {
...
contract EndToEndRevenueStreamingTests is RDTTestBase {
    ...
    function test_vesting_singleSchedule_fuzz(
        uint256 depositAmount,
        ...
        public
    {
        ...
        vestingPeriod =
            constrictToRange(vestingPeriod, 10 seconds, 100_000 days) / 10 * 10; // Must be divisible by 10 for for loop 10% increment calculations // TODO: Add a zero case test
        ...
        for (uint256 i = 1; i < 10; ++i) {
            ...
            uint256 expectedTotalAssets =
                depositAmount + expectedRate * (block.timestamp - START) / 1e30;
...
contract MintTests is RDTSuccessTestBase {
    ...
    function testFuzz_mint_singleUser_preVesting(uint256 mintAmount_)
        public
    {
        mintAmount_ = constrictToRange(mintAmount_, 1, 1e29);
        ...
        asset_allowance_staker_rdToken_change =
            -_toInt256(rdToken.convertToAssets(mintAmount_));
        ...
        public
    {
        ...
        uint256 expectedAssets =
            mintAmount_ * rdToken.totalAssets() / rdToken.totalSupply();
        uint256 vestedAmount = rdToken.issuanceRate() * warpTime_ / 1e30;
    ...
    function testFuzz_mint_singleUser_postVesting(
        uint256 initialAmount_,
        uint256 mintAmount_,
        uint256 vestingAmount_
    )
        ...
        public
    {
        ...
        for (uint256 i = 1; i < 11; ++i) {
            uint256 mintAmount =
                uint256(keccak256(abi.encodePacked(mintSeed_, i)));
            uint256 warpTime =
                uint256(keccak256(abi.encodePacked(warpSeed_, i)));
            ...
            uint256 expectedAssets =
                mintAmount * rdToken.totalAssets() / rdToken.totalSupply();
            uint256 vestedAmount = rdToken.issuanceRate() * warpTime / 1e30;
...
contract MintWithPermitFailureTests is RDTTestBase {
    ...
    function test_mintWithPermit_zeroAddress() public {
        ...
        (uint8 v, bytes32 r, bytes32 s) =
        _getValidPermitSignature(staker, address(rdToken), maxAssets, deadline, stakerPrivateKey);
    ...
    function test_mintWithPermit_notStakerSignature() public {
        ...
        (uint8 v, bytes32 r, bytes32 s) =
        _getValidPermitSignature(notStaker, address(rdToken), maxAssets, deadline, notStakerPrivateKey);
    ...
    function test_mintWithPermit_insufficientPermit() public {
        ...
        (uint8 v, bytes32 r, bytes32 s) =
        _getValidPermitSignature(staker, address(rdToken), maxAssets - 1, deadline, stakerPrivateKey);
...
contract MintWithPermitTests is RDTSuccessTestBase {
    ...
    function testFuzz_mintWithPermit_singleUser_preVesting(uint256 mintAmount_)
        public
    {
        mintAmount_ = constrictToRange(mintAmount_, 1, 1e29);
...
contract RedeemCallerNotOwnerTests is RDTSuccessTestBase {
        ...
        public
    {
        ...
        uint256 expectedWithdrawnFunds =
            redeemAmount_ * rdToken.totalAssets() / rdToken.totalSupply();
        uint256 vestedAmount = rdToken.issuanceRate() * warpTime_ / 1e30;
        ...
        rdToken_freeAssets_change =
            _toInt256(vestedAmount) - _toInt256(expectedWithdrawnFunds); // freeAssets gets updated to reflects 5e18 vested tokens during withdraw
        rdToken_convertToAssets_change = 0;
        rdToken_convertToShares_change = 0;
        rdToken_issuanceRate_change = 0;
        rdToken_lastUpdated_change = _toInt256(warpTime_);
        ...
        public
    {
        ...
        rdToken_totalAssets_change = -_toInt256(expectedWithdrawnFunds);
        rdToken_freeAssets_change =
            _toInt256(vestingAmount_) - _toInt256(expectedWithdrawnFunds); // freeAssets gets updated to reflects 5e18 vested tokens during withdraw
        rdToken_convertToAssets_change = 0;
        rdToken_convertToShares_change = 0;
        rdToken_issuanceRate_change = 0;
        ...
        public
    {
        ...
        initWarpTime =
            constrictToRange(initWarpTime,   1 seconds,             100 days);
        vestingPeriod_ =
            constrictToRange(vestingPeriod_, 1 days + initWarpTime, 1e29);
        ...
        for (uint256 i; i < iterations_; ++i) {
            ...
            uint256 redeemAmount =
                uint256(keccak256(abi.encodePacked(redeemSeed_, i)));
            uint256 warpTime =
                uint256(keccak256(abi.encodePacked(warpSeed_,   i)));

            redeemAmount =
            constrictToRange(redeemAmount, 1, rdToken.balanceOf(address(stakers[i])));
            warpTime =
            constrictToRange(warpTime,     0, (vestingPeriod_ - initWarpTime) / iterations_);
        ...
        public
    {
        ...
        for (uint256 i; i < iterations_; ++i) {
            ...
            constrictToRange(redeemAmount, 1, rdToken.balanceOf(address(stakers[i])));
            warpTime =
            constrictToRange(warpTime,     0, (vestingPeriod_ - 12 hours) / iterations_);
...
contract RedeemFailureTests is RDTTestBase {
    ...
    function test_redeem_burnUnderflow_totalAssetsGtTotalSupply_explicitValues()
        public
    {
...
contract RedeemRevertOnTransfers is RDTTestBase {
    ...
    function setUp() public virtual override {
        revertingAsset =
            new MockRevertingERC20("MockToken", "MT", 18, address(123));
        rdToken =
        new RDT("Revenue Distribution Token", "RDT", address(this), address(revertingAsset), 1e30);
        staker = new Staker();

        vm.warp(START); // Warp to non-zero timestamp
    }

    function test_redeem_revertOnTransfer(
        uint256 depositAmount_,
        uint256 redeemAmount_
    )
        public
...
contract RedeemTests is RDTSuccessTestBase {
    ...
    function testFuzz_redeem_singleUser_preVesting(
        uint256 depositAmount_,
        uint256 redeemAmount_
    )
        public
        ...
        public
    {
        depositAmount_ = constrictToRange(depositAmount_, 1,         1e29);
        redeemAmount_ =
            constrictToRange(redeemAmount_,  1,         depositAmount_);
        vestingAmount_ = constrictToRange(vestingAmount_, 1,         1e29);
        ...
        public
    {
        ...
        for (uint256 i; i < iterations_; ++i) {
            ...
            rdToken_totalAssets_change = -_toInt256(expectedWithdrawnFunds);
            rdToken_freeAssets_change =
                _toInt256(vestedAmount) - _toInt256(expectedWithdrawnFunds);
            rdToken_convertToAssets_change = 0;
            rdToken_convertToShares_change = 0;
            rdToken_issuanceRate_change = 0;
            rdToken_lastUpdated_change = _toInt256(warpTime);
        ...
        public
    {
        ...
        for (uint256 i; i < 10; ++i) {
            ...
            constrictToRange(redeemAmount, 1, rdToken.balanceOf(address(stakers[i])));
            warpTime =
                constrictToRange(warpTime,     0, (vestingPeriod_ - 12 hours) / 10);
...
contract UpdateVestingScheduleTests is RDTTestBase {
    ...
    function test_updateVestingSchedule_sameTime_longerVesting_higherRate()
        public
    {
        _transferAndUpdateVesting(address(asset), address(rdToken), 1000, 100 seconds);
        assertEq(rdToken.issuanceRate(),        10e30); // 1000 / 100 seconds = 10 tokens per second
    ...
    function test_updateVestingSchedule_sameTime_longerVesting_lowerRate()
        public
    {
        _transferAndUpdateVesting(address(asset), address(rdToken), 1000, 100 seconds);
        assertEq(rdToken.issuanceRate(),        10e30); // 1000 / 100 seconds = 10 tokens per second
    ...
    function test_updateVestingSchedule_diffTime_longerVesting_higherRate()
        public
    {
        _transferAndUpdateVesting(address(asset), address(rdToken), 1000, 100 seconds); // 10 tokens per second
    ...
    function test_updateVestingSchedule_diffTime_longerVesting_lowerRate()
        public
    {
        _transferAndUpdateVesting(address(asset), address(rdToken), 1000, 100 seconds); // 10 tokens per second
...
contract WithdrawCallerNotOwnerTests is RDTSuccessTestBase {
        ...
        public
    {
        ...
        uint256 maxWithdrawAmount =
            depositAmount_ * rdToken.totalAssets() / rdToken.totalSupply();

        withdrawAmount_ =
            constrictToRange(withdrawAmount_, 1, maxWithdrawAmount);
        ...
        rdToken_allowance_staker_caller_change =
            -_toInt256(expectedSharesBurned);
        rdToken_balanceOf_staker_change = -_toInt256(expectedSharesBurned);
        rdToken_totalSupply_change = -_toInt256(expectedSharesBurned);
        rdToken_totalAssets_change = -_toInt256(withdrawAmount_);
        rdToken_freeAssets_change =
            _toInt256(vestedAmount) - _toInt256(withdrawAmount_); // freeAssets gets updated to reflects 5e18 vested tokens during withdraw
        rdToken_convertToAssets_change = 0;
        rdToken_convertToShares_change = 0;
        rdToken_issuanceRate_change = 0;
        rdToken_lastUpdated_change = _toInt256(warpTime_);
        ...
        public
    {
        ...
        rdToken_totalAssets_change = -_toInt256(withdrawAmount_);
        rdToken_freeAssets_change =
            _toInt256(vestingAmount_) - _toInt256(withdrawAmount_);
        rdToken_convertToAssets_change = 0;
        rdToken_convertToShares_change = 0;
        rdToken_issuanceRate_change = -_toInt256(rdToken.issuanceRate());
        rdToken_lastUpdated_change = _toInt256(vestingPeriod_ + 1 seconds);
        ...
        public
    {
        ...
        for (uint256 i; i < iterations_; ++i) {
            ...
            uint256 withdrawAmount =
                uint256(keccak256(abi.encodePacked(withdrawSeed_, i)));
            uint256 warpTime =
                uint256(keccak256(abi.encodePacked(warpSeed_,     i)));

            {
                uint256 maxWithdrawAmount =
                    rdToken.balanceOf(address(stakers[i]))
                    * rdToken.totalAssets()
                    / rdToken.totalSupply();
                withdrawAmount =
                    constrictToRange(withdrawAmount, 1, maxWithdrawAmount);
            }
            ...
            uint256 expectedSharesBurned =
                rdToken.previewWithdraw(withdrawAmount);
            uint256 vestedAmount = rdToken.issuanceRate() * warpTime / 1e30;
            ...
            rdToken_freeAssets_change =
                _toInt256(vestedAmount) - _toInt256(withdrawAmount); // freeAssets gets updated to reflects 5e18 vested tokens during withdraw
            rdToken_convertToAssets_change = 0;
            rdToken_convertToShares_change = 0;
            rdToken_issuanceRate_change = 0;
            rdToken_lastUpdated_change = _toInt256(warpTime);
        ...
        public
    {
        ...
        for (uint256 i; i < 10; ++i) {
            ...
            uint256 maxWithdrawAmount = rdToken.balanceOf(address(stakers[i]))
                * rdToken.totalAssets()
                / rdToken.totalSupply();
...
contract WithdrawFailureTests is RDTTestBase {
    ...
    function test_withdraw_burnUnderflow_totalAssetsGtTotalSupply_explicitValues(
    )
        public
    {
        uint256 depositAmount = 100e18;
        ...
        uint256 maxWithdrawAmount =
            rdToken.previewRedeem(rdToken.balanceOf(address(staker))); // TODO
    ...
    function test_withdraw_callerNotOwner_infiniteApprovalForCaller()
        public
    {
        Staker shareOwner = new Staker();
        Staker notShareOwner = new Staker();
...
contract WithdrawRevertOnTransfers is RDTTestBase {
    ...
    function test_withdraw_revertOnTransfer(
        uint256 depositAmount_,
        uint256 withdrawAmount_
    )
        public
...
contract WithdrawTests is RDTSuccessTestBase {
    ...
    function testFuzz_withdraw_singleUser_preVesting(
        uint256 depositAmount_,
        uint256 withdrawAmount_
    )
        public
        ...
        public
    {
        ...
        rdToken_totalAssets_change = -_toInt256(withdrawAmount_);
        rdToken_freeAssets_change =
            _toInt256(vestingAmount_) - _toInt256(withdrawAmount_); // freeAssets gets updated to reflects 5e18 vested tokens during withdraw
        rdToken_convertToAssets_change = 0;
        rdToken_convertToShares_change = 0;
        rdToken_issuanceRate_change = -_toInt256(rdToken.issuanceRate());
        rdToken_lastUpdated_change = _toInt256(vestingPeriod_ + 1 seconds);
