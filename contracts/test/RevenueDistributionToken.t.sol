...
contract RDTTestBase is TestUtils {
    ...
    function _depositAsset(address asset_, address staker_, uint256 depositAmount_) internal returns (uint256 shares) {
        MockERC20(asset_).mint(staker_, depositAmount_);
        Staker(staker_).erc20_approve(asset_, address(rdToken), depositAmount_);
        shares = Staker(staker_).rdToken_deposit(address(rdToken), depositAmount_);
    }
...
contract RDTSuccessTestBase is RDTTestBase {
    ...
    function _assertMintWithPermit(address staker_, uint256 stakerPrivateKey_, uint256 mintAmount_, bool fuzzed_) internal {
        ...
        vm.prank(staker_);
        uint256 shares = rdToken.mintWithPermit(mintAmount_, staker_, maxAssets, block.timestamp, v, r, s);
...
contract DepositWithPermitTest is RDTSuccessTestBase {
    ...
    function testFuzz_depositWithPermit_multiUser_postVesting(uint256 initialAmount_, uint256 vestingAmount_, uint256 vestingPeriod_, bytes32 seed_) external {
        initialAmount_ = constrictToRange(initialAmount_, 1, 1e29);
        vestingAmount_ = constrictToRange(vestingAmount_, 1, 1e29);
        vestingPeriod_ = constrictToRange(vestingPeriod_, 1, 1e29);
...
contract RedeemAndWithdrawTest is RDTSuccessTestBase {
    ...
    function test_withdraw_singleUser_preVesting() external {
        ...
        _assertWithdraw(staker, withdrawAmount, false);
    }
    ...
    function test_withdraw_singleUser_postVesting() external {
        ...
        vm.warp(block.timestamp + 201 seconds);  // Vest 10e18 tokens

        uint256 withdrawAmount          = 20e18;
        rdToken_balanceOf_staker_change = -18_181818181818181819;  // 20 / 1.1
        rdToken_totalSupply_change      = -18_181818181818181819;  // 20 / 1.1
        rdToken_freeAssets_change       = -20e18;  // freeAssets gets updated to reflects 10e18 vested tokens during withdraw
        rdToken_totalAssets_change      = -20e18;
        rdToken_convertToAssets_change  = 0;
        rdToken_convertToShares_change  = 0;
        rdToken_issuanceRate_change     = 0;
    ...
    function testFuzz_withdraw_singleUser_withVesting(
        uint256 depositAmount_,
        uint256 withdrawAmount_,
        uint256 vestingAmount_,
        uint256 vestingPeriod_,
    ...
    function testFuzz_withdraw_singleUser_postVesting(
        ...
        vestingAmount_  = constrictToRange(vestingAmount_,  1, 1e29);
        vestingPeriod_  = constrictToRange(vestingPeriod_,  1, 100 days);
        warpTime_       = constrictToRange(warpTime_,       vestingPeriod_ + 1, 200 days);
        ...
        rdToken_totalSupply_change      = - _toInt256(expectedSharesBurned);
        rdToken_totalAssets_change      = - _toInt256(withdrawAmount_);
        rdToken_freeAssets_change       = _toInt256(vestingAmount_ * warpTime_ / vestingPeriod_) - _toInt256(withdrawAmount_);
        rdToken_convertToAssets_change  = 0;
        rdToken_convertToShares_change  = 0;
        rdToken_issuanceRate_change     = 0;
        rdToken_lastUpdated_change      = _toInt256(warpTime_);
