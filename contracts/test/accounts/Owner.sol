...
contract Owner {
    ...
    function rdToken_setPendingOwner(address rdt_, address pendingOwner_)
        external
    {
        IRDT(rdt_).setPendingOwner(pendingOwner_);
    }

    function rdToken_updateVestingSchedule(address rdt_, uint256 vestingPeriod_)
        external
    ...
    function erc20_transfer(address token_, address receiver_, uint256 amount_)
        external
